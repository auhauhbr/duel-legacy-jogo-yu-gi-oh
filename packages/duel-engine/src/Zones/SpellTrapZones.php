...
final readonly class SpellTrapZones
{
    /**
    ...
    public static function empty(int $capacity): self
    {
        if ($capacity < 0) {
            throw new InvalidArgumentException(
                "A capacidade das Zonas de Magia e Armadilha não pode ser negativa: {$capacity}.",
            );
        }
    ...
    private function assertValidIndex(int $index): void
    {
        if ($index < 0 || $index >= $this->capacity()) {
            throw new InvalidArgumentException(
                "Índice das Zonas de Magia e Armadilha fora do intervalo: {$index}.",
            );
        }
    }
    ...
    private static function assertSlots(array $slots): void
    {
        ...
        foreach ($slots as $card) {
            ...
            if (isset($seenIds[$key])) {
                throw new InvalidArgumentException(
                    "CardInstanceId duplicado nas Zonas de Magia e Armadilha: {$card->id->value}.",
                );
            }
